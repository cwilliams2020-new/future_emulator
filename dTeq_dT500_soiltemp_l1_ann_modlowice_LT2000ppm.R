...
for (i in Exp_list_LC_tdvp) {
	...
	var_nam=paste(Exp, i, '_l1', sep = "")
	soiltemp = ncvar_get(nc, var_nam)
	nam = paste("soiltemp_tdvp", i, "_kdat", sep = "")
	assign(nam, soiltemp)
	nc_close(nc)
	rm(nc, soiltemp)
}
...
soiltemp_tdvp_k_all = matrix(0, nrow = length(soiltemp_tdvpc_kdat), ncol = Exp_num_tdvp)
j = 1


for (i in Exp_list_LC_tdvp) {
  nam = paste("soiltemp_tdvp", i, "_kdat", sep = "")
  nam_new = paste("soiltemp_tdvp", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  soiltemp_tdvp_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvp = soiltemp_tdvp_k_all
...
for (i in Exp_list_LC_tdvq) {
	...
	var_nam=paste(Exp, i, '_l1', sep = "")
	soiltemp = ncvar_get(nc, var_nam)
	nam = paste("soiltemp_tdvq", i, "_kdat", sep = "")
	assign(nam, soiltemp)
	nc_close(nc)
	rm(nc, soiltemp)
}
...
soiltemp_tdvq_k_all = matrix(0, nrow = length(soiltemp_tdvqc_kdat), ncol = Exp_num_tdvq)
j = 1


for (i in Exp_list_LC_tdvq) {
  nam = paste("soiltemp_tdvq", i, "_kdat", sep = "")
  nam_new = paste("soiltemp_tdvq", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  soiltemp_tdvq_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvq = soiltemp_tdvq_k_all
