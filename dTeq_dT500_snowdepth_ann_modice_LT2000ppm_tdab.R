...
for (i in Exp_list_LC_tdum) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_snowdepth_mm_srf_ann_", Exp, ".nc", sep="")))
...
# --------------------------------------------------
# VECTORIZE SNOW DEPTH MATRICES (BY COLUMN (LATITUDE))
...
for (i in Exp_list_LC_tdvo) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_snowdepth_mm_srf_ann_", Exp, "_LT2000ppm.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_LC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_snowdepth_mm_srf_ann_", Exp, "28k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_snowdepth_mm_srf_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_snowdepth_mm_srf_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
