...
cont_paramdat = read.table(file.path('C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Emulator/2015_Bristol_5D_v001/orig/Input/Samp_orbits_tdvp.res'), sep=" ", header=TRUE)
...
for (i in Exp_list_LC_tdvp) {
	...
	var_nam=paste(Exp, i, sep = "")
	precip = ncvar_get(nc, var_nam)
	nam = paste("precip_tdvp", i, "_kdat", sep = "")
	assign(nam, precip)
	nc_close(nc)
	rm(nc, precip)
}
...
precip_tdvp_k_all = matrix(0, nrow = length(precip_tdvpc_kdat), ncol = Exp_num_tdvp)
j = 1


for (i in Exp_list_LC_tdvp) {
  nam = paste("precip_tdvp", i, "_kdat", sep = "")
  nam_new = paste("precip_tdvp", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  precip_tdvp_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvp = precip_tdvp_k_all
...
cont_paramdat = read.table(file.path('C:/Users/cw18831/OneDrive - University of Bristol/Documents/Research/SKB/SKB_Alan_Code_DJL/PosivaSKB/PosivaSKB/Emulator/2015_Bristol_5D_v001/orig/Input/Samp_orbits_tdvq_LT2000ppm.res'), sep=" ", header=TRUE)
...
for (i in Exp_list_LC_tdvq) {
	...
	var_nam=paste(Exp, i, sep = "")
	precip = ncvar_get(nc, var_nam)
	nam = paste("precip_tdvq", i, "_kdat", sep = "")
	assign(nam, precip)
	nc_close(nc)
	rm(nc, precip)
}
...
precip_tdvq_k_all = matrix(0, nrow = length(precip_tdvqc_kdat), ncol = Exp_num_tdvq)
j = 1


for (i in Exp_list_LC_tdvq) {
  nam = paste("precip_tdvq", i, "_kdat", sep = "")
  nam_new = paste("precip_tdvq", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  precip_tdvq_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvq = precip_tdvq_k_all
