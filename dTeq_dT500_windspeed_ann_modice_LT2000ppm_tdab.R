...
for (i in Exp_list_LC_tdum) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_windspeed_mm_10m_ann_", Exp, ".nc", sep="")))
...
# --------------------------------------------------
# VECTORIZE WIND SPEED MATRICES (BY COLUMN (LATITUDE))
...
model_output_tdum <- array(model_output_tdum, c(96, 72, Exp_num_tdum ))
...
for (i in Exp_list_LC_tdvo) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_windspeed_mm_10m_ann_", Exp, "_LT2000ppm.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude_1")
	lons_dat = ncvar_get(nc, "longitude_1")
...
model_output_tdvo <- array(model_output_tdvo, c(96, 72, Exp_num_tdvo ))
...
for (i in Exp_list_LC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_windspeed_mm_10m_ann_", Exp, "28k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude_1")
	lons_dat = ncvar_get(nc, "longitude_1")
...
for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_windspeed_mm_10m_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude_1")
	lons_dat = ncvar_get(nc, "longitude_1")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_windspeed_mm_10m_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude_1")
	lons_dat = ncvar_get(nc, "longitude_1")
...
model_output_tdab <- array(model_output_tdab, c(96, 72, Exp_num_tdab ))
...
model_output_modice_tdab <- array(0, c(96, 72, (Exp_num_tdum+Exp_num_tdvo+Exp_num_tdab)))
