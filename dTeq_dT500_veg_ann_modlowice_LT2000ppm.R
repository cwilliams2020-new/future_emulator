...
for (i in Exp_list_LC_tdvp) {
	...
	var_nam=paste(Exp, i, sep = "")
	veg = ncvar_get(nc, var_nam)
	nam = paste("veg_tdvp", i, "_kdat", sep = "")
	assign(nam, veg)
	nc_close(nc)
	rm(nc, veg)
}
...
veg_tdvp_k_all = matrix(0, nrow = length(veg_tdvpc_kdat), ncol = Exp_num_tdvp)
j = 1


for (i in Exp_list_LC_tdvp) {
  nam = paste("veg_tdvp", i, "_kdat", sep = "")
  nam_new = paste("veg_tdvp", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  veg_tdvp_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvp = veg_tdvp_k_all
...
for (i in Exp_list_LC_tdvq) {
	...
	var_nam=paste(Exp, i, sep = "")
	veg = ncvar_get(nc, var_nam)
	nam = paste("veg_tdvq", i, "_kdat", sep = "")
	assign(nam, veg)
	nc_close(nc)
	rm(nc, veg)
}
...
veg_tdvq_k_all = matrix(0, nrow = length(veg_tdvqc_kdat), ncol = Exp_num_tdvq)
j = 1


for (i in Exp_list_LC_tdvq) {
  nam = paste("veg_tdvq", i, "_kdat", sep = "")
  nam_new = paste("veg_tdvq", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  veg_tdvq_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvq = veg_tdvq_k_all
