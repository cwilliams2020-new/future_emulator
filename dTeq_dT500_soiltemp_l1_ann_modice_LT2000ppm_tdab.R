...
for (i in Exp_list_LC_tdum) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_soiltemp_mm_soil_ann_", Exp, ".nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, '_l1', sep = "")
	soiltemp = ncvar_get(nc, var_nam)
	nam = paste("soiltemp_tdum", i, "_kdat", sep = "")
	assign(nam, soiltemp)
	nc_close(nc)
	rm(nc, soiltemp)
}
...
soiltemp_tdum_k_all = matrix(0, nrow = length(soiltemp_tdumc_kdat), ncol = Exp_num_tdum)
j = 1


for (i in Exp_list_LC_tdum) {
  nam = paste("soiltemp_tdum", i, "_kdat", sep = "")
  nam_new = paste("soiltemp_tdum", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  soiltemp_tdum_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdum = soiltemp_tdum_k_all
...
for (i in Exp_list_LC_tdvo) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_soiltemp_mm_soil_ann_", Exp, "_LT2000ppm.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, '_l1', sep = "")
	soiltemp = ncvar_get(nc, var_nam)
	nam = paste("soiltemp_tdvo", i, "_kdat", sep = "")
	assign(nam, soiltemp)
	nc_close(nc)
	rm(nc, soiltemp)
}
...
soiltemp_tdvo_k_all = matrix(0, nrow = length(soiltemp_tdvoc_kdat), ncol = Exp_num_tdvo)
j = 1


for (i in Exp_list_LC_tdvo) {
  nam = paste("soiltemp_tdvo", i, "_kdat", sep = "")
  nam_new = paste("soiltemp_tdvo", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  soiltemp_tdvo_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvo = soiltemp_tdvo_k_all
...
for (i in Exp_list_LC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_soiltemp_mm_soil_ann_", Exp, "28k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	...
	nam = paste("soiltemp_tdab", i, "_kdat", sep = "")
	assign(nam, soiltemp_l1)
	nc_close(nc)
	rm(nc, soiltemp, soiltemp_l1)
}


for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_soiltemp_mm_soil_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_soiltemp_mm_soil_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
soiltemp_tdab_k_all = matrix(0, nrow = length(soiltemp_tdabc_kdat), ncol = Exp_num_tdab)
j = 1


for (i in Exp_list_LC_tdab) {
  nam = paste("soiltemp_tdab", i, "_kdat", sep = "")
  nam_new = paste("soiltemp_tdab", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  soiltemp_tdab_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdab = soiltemp_tdab_k_all
