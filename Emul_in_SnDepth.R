...
## Calibrate final using snowdepth_mm_srf #############################################################
###################################################################################################
...
# load data
source('../Data/Data - dTeq dT500/dTeq_dT500_snowdepth_ann_modice_LT2000ppm_tdab.R')
source('../Data/Data - dTeq dT500/dTeq_dT500_snowdepth_ann_modlowice_LT2000ppm.R')

print('Emulator_snowdepth')

# Clear some unnecessary variables, except those listed here
source("Emulator_remove_vars1.R")

source('../Data/Data - Climate NC/Tdst_snowdepth_mm_srf.R')
...
# Convert from mm (kg/m2) to m (tdab and tdst only)

model_output_modice_tdab[,,(Exp_num_tdum+Exp_num_tdvo+1):(Exp_num_tdum+Exp_num_tdvo+Exp_num_tdab)] = model_output_modice_tdab[,,(Exp_num_tdum+Exp_num_tdvo+1):(Exp_num_tdum+Exp_num_tdvo+Exp_num_tdab)]/1000
model_output_tdst[,,] = model_output_tdst[,,]/1000


# Add tdstb on so that absolute snow depths are emulated (tdum only)

model_output_modice_tdab_abs = model_output_modice_tdab
model_output_modlowice_abs = model_output_modlowice

for (i in 1:(Exp_num_tdum+Exp_num_tdvo)){
  model_output_modice_tdab_abs[,,i] = model_output_modice_tdab[,,i] + model_output_tdst[,,1]
}

for (i in 1:(Exp_num_tdum+Exp_num_tdvo+Exp_num_tdvp+Exp_num_tdvq)){
  model_output_modlowice_abs[,,i] = model_output_modlowice[,,i] + model_output_tdst[,,1]
}

model_output_modice_tdab_abs[model_output_modice_tdab_abs < 0] = 0
model_output_modlowice_abs[model_output_modlowice_abs < 0] = 0
model_output_tdst[model_output_tdst < 0] = 0


# Cap snow at 10 m

model_output_modice_tdab_capped = model_output_modice_tdab_abs
model_output_modlowice_capped = model_output_modlowice_abs
model_output_tdst_capped = model_output_tdst

model_output_modice_tdab_capped[model_output_modice_tdab_capped > 10] = 10
model_output_modlowice_capped[model_output_modlowice_capped > 10] = 10
model_output_tdst_capped[model_output_tdst_capped > 10] = 10
...
Y_gl = model_output_modice_tdab_capped
Y_ig = model_output_modlowice_capped
