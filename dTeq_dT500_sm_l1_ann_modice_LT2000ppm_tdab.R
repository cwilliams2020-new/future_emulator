...
for (i in Exp_list_LC_tdum) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_sm_mm_soil_ann_", Exp, ".nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, '_l1', sep = "")
	sm = ncvar_get(nc, var_nam)
	nam = paste("sm_tdum", i, "_kdat", sep = "")
	assign(nam, sm)
	nc_close(nc)
	rm(nc, sm)
}
...
# --------------------------------------------------
# VECTORIZE SOIL MOISTURE MATRICES (BY COLUMN (LATITUDE))

sm_tdum_k_all = matrix(0, nrow = length(sm_tdumc_kdat), ncol = Exp_num_tdum)
j = 1


for (i in Exp_list_LC_tdum) {
  nam = paste("sm_tdum", i, "_kdat", sep = "")
  nam_new = paste("sm_tdum", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdum_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdum = sm_tdum_k_all
...
for (i in Exp_list_LC_tdvo) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_sm_mm_soil_ann_", Exp, "_LT2000ppm.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, '_l1', sep = "")
	sm = ncvar_get(nc, var_nam)
	nam = paste("sm_tdvo", i, "_kdat", sep = "")
	assign(nam, sm)
	nc_close(nc)
	rm(nc, sm)
}
...
sm_tdvo_k_all = matrix(0, nrow = length(sm_tdvoc_kdat), ncol = Exp_num_tdvo)
j = 1


for (i in Exp_list_LC_tdvo) {
  nam = paste("sm_tdvo", i, "_kdat", sep = "")
  nam_new = paste("sm_tdvo", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdvo_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvo = sm_tdvo_k_all
...
for (i in Exp_list_LC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_sm_mm_soil_ann_", Exp, "28k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_sm_mm_soil_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_sm_mm_soil_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
sm_tdab_k_all = matrix(0, nrow = length(sm_tdabc_kdat), ncol = Exp_num_tdab)
j = 1


for (i in Exp_list_LC_tdab) {
  nam = paste("sm_tdab", i, "_kdat", sep = "")
  nam_new = paste("sm_tdab", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdab_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdab = sm_tdab_k_all
