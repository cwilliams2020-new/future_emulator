...
for (i in Exp_list) {
	...
	sm = ncvar_get(nc, "sm_mm_soil")
	sm_l1 = sm[,,1]
	nam = paste("sm_tdst", i, "_kdat", sep = "")
	assign(nam, sm_l1)
	nc_close(nc)
	rm(nc, sm)
}
...
sm_tdst_k_all = matrix(0, nrow = length(sm_tdstb_kdat), ncol = 5)
j = 1


for (i in Exp_list) {
  nam = paste("sm_tdst", i, "_kdat", sep = "")
  nam_new = paste("sm_tdst", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdst_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdst = sm_tdst_k_all

# and put then on a grid format

model_output_tdst <- array(model_output_tdst, c(96, 73, 5 ))

rm(list=ls()[(ls() %in% c('Exp','Exp_list','cont_paramdat','cont_param_dim','sm_tdst_k_all'))])
