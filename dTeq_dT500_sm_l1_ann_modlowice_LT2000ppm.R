...
# --------------------------------------------------
# VECTORIZE SOIL MOISTUREERATURE MATRICES (BY COLUMN (LATITUDE))
...
sm_tdvp_k_all = matrix(0, nrow = length(sm_tdvpc_kdat), ncol = Exp_num_tdvp)
j = 1


for (i in Exp_list_LC_tdvp) {
  nam = paste("sm_tdvp", i, "_kdat", sep = "")
  nam_new = paste("sm_tdvp", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdvp_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvp = sm_tdvp_k_all
...
sm_tdvq_k_all = matrix(0, nrow = length(sm_tdvqc_kdat), ncol = Exp_num_tdvq)
j = 1


for (i in Exp_list_LC_tdvq) {
  nam = paste("sm_tdvq", i, "_kdat", sep = "")
  nam_new = paste("sm_tdvq", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  sm_tdvq_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvq = sm_tdvq_k_all
