...
for (i in Exp_list_LC_tdum) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_TLAI_PFT_snp_srf_ann_", Exp, ".nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, sep = "")
	veg = ncvar_get(nc, var_nam)
	nam = paste("veg_tdum", i, "_kdat", sep = "")
	assign(nam, veg)
	nc_close(nc)
	rm(nc, veg)
}
...
veg_tdum_k_all = matrix(0, nrow = length(veg_tdumc_kdat), ncol = Exp_num_tdum)
j = 1


for (i in Exp_list_LC_tdum) {
  nam = paste("veg_tdum", i, "_kdat", sep = "")
  nam_new = paste("veg_tdum", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  veg_tdum_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdum = veg_tdum_k_all
...
for (i in Exp_list_LC_tdvo) {
	nc = nc_open(file.path('..','orig','Output','dTeq','LT2000', paste("dTeq_TLAI_PFT_snp_srf_ann_", Exp, "_LT2000ppm.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, sep = "")
	veg = ncvar_get(nc, var_nam)
	nam = paste("veg_tdvo", i, "_kdat", sep = "")
	assign(nam, veg)
	nc_close(nc)
	rm(nc, veg)
}
...
veg_tdvo_k_all = matrix(0, nrow = length(veg_tdvoc_kdat), ncol = Exp_num_tdvo)
j = 1


for (i in Exp_list_LC_tdvo) {
  nam = paste("veg_tdvo", i, "_kdat", sep = "")
  nam_new = paste("veg_tdvo", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  veg_tdvo_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdvo = veg_tdvo_k_all
...
for (i in Exp_list_LC_tdab) {
	...
	var_nam=paste(Exp, i, sep = "")
	veg = ncvar_get(nc, var_nam)
	veg=array(veg[,,1:nl], c(96, 73, nl))
	nam = paste("veg_tdab", i, "_kdat", sep = "")
	assign(nam, veg)
	nc_close(nc)
	rm(nc, veg)
}
...
for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_fracPFTs_snp_srf_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_fracPFTs_snp_srf_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_LC_tdab) {
  nam = paste("veg_tdab", i, "_kdat", sep = "")
  data = array(data=eval(parse(text=nam)), c(96, 73, nl))
  data = replace(data, data == 2.00000004008175e+20, NA)
  assign(nam, data)
  rm(data)
}
...
for (i in Exp_list_LC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_LAI_PFT_mm_srf_ann_", Exp, "28k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
	
	var_nam=paste(Exp, i, sep = "")
	LAI = ncvar_get(nc, var_nam)
	LAI=array(LAI[,,1:nl], c(96, 73, nl))
	nam = paste("LAI_tdab", i, "_kdat", sep = "")
	assign(nam, LAI)
	nc_close(nc)
	rm(nc, LAI)
}
...
for (i in Exp_list_UC_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_LAI_PFT_mm_srf_ann_", Exp, "80k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
for (i in Exp_list_NUM_tdab) {
	nc = nc_open(file.path('..','orig','Output','Singarayer + Valdes 2010', paste("dT500_LAI_PFT_mm_srf_ann_", Exp, "120k.nc", sep="")))
	lats_dat = ncvar_get(nc, "latitude")
	lons_dat = ncvar_get(nc, "longitude")
...
# --------------------------------------------------
# CALCULATE TOTAL LAI (VEG X LAI FOR EACH LEVEL, THEN SUM)

for (i in Exp_list_LC_tdab) {
  nam1 = paste("veg_tdab", i, "_kdat", sep = "")
  nam2 = paste("LAI_tdab", i, "_kdat", sep = "")
  nam3 = paste("TLAI_tdab", i, "_kdat", sep = "")
  data1 = array(data=eval(parse(text=nam1)), c(96, 73, nl))
  data2 = array(data=eval(parse(text=nam2)), c(96, 73, nl))
  for (n in 1:nl) {
    data1[,,n] = data1[,,n] * data2[,,n]
  }
  data3 = rowSums(data1[,,], dims = 2)
  assign(nam3, data3)
...
TLAI_tdab_k_all = matrix(0, nrow = length(TLAI_tdabc_kdat), ncol = Exp_num_tdab)
j = 1


for (i in Exp_list_LC_tdab) {
  nam = paste("TLAI_tdab", i, "_kdat", sep = "")
  nam_new = paste("TLAI_tdab", i, "_k_vec", sep = "")
  data = matrix(data=eval(parse(text=nam)), ncol = 73)
  data_new = as.vector(data)
  assign(nam_new, data_new)
  TLAI_tdab_k_all[,j] = data_new
  rm(data)
  j = j + 1
}
...
model_output_tdab = TLAI_tdab_k_all
